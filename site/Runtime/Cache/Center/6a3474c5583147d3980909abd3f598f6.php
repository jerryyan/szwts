        ...
        <div class="scxx_bg" style="margin-left:10px;">       	
           	<table border="0">
	            ...
	            <?php if(($tempData[num] > 0)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="tr_01">
	                <td class="t_2" align="center" valign="middle"><?php echo ($v['name']); ?></td>
	                <td class="t_2" align="center" valign="middle">
	                <?php if(($v[status] == 1)): ?>已绑定
	                <?php else: ?>
	               	 	未绑定<?php endif; ?>
	                </td>
	                <td class="t_2" align="center" valign="middle"><?php echo (($v['username'])?($v['username']):'-'); ?></td>
	                <td class="t_2" align="center" valign="middle"><?php echo (($v['type'])?($v['type']):'-'); ?></td>
	                <td class="t_2" align="center" valign="middle"><?php echo ((date('Y-m-d H:i:s', $v['addtime']))?(date('Y-m-d H:i:s', $v['addtime'])):'-'); ?></td>
	            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
	            <?php else: ?>
	            <tr class="tr_01">
	            	<td class="t_2">暂无数据</td>
	            </tr><?php endif; ?>
	       	</table>
	   	</div>
	   	<!-- 分页 开始-->
