...
<div id="content">
	...
	<div class="content_right">
        ...
        <div class="rznr">
			...
			<div class="form_nr">
				<div class="input_nr">
					<span style="width:98px; text-align:right;">
						<a style=" float:left; color:#F00; height:28px; font:14px '微软雅黑'; padding-top:2px; margin-left:16px;">*</a>原提现密码
					</span>
				...
				<div class="input_nr">
					<span style="width:98px; text-align:right;">
						<a style=" float:left; color:#F00; height:28px; font:14px '微软雅黑'; padding-top:2px; margin-left:16px;">*</a>新提现密码
					</span>
                ...
                <div class="input_nr">
                	<span style="width:98px; text-align:right;">
                		<a style=" float:left; color:#F00; height:28px; font:14px '微软雅黑'; padding-top:2px;">*</a>确认提现密码
                	</span>
				...
				<input id="modifyPayPwdBtn" type="button" value="" class="input_btn sj_btn" />
...
$(function(){
	$("#oldpwd").focusin(function(){
		$("#js_oldpwd_tips").css({'color':'#666'}).text('请输入原始提现密码');
	}).focusout(function(){
		var _this = $(this),
			v = _this.val();
		checkOldpwd(v);
	...
	$("#newpwd").focusin(function(){
		$("#js_newpwd_tips").css({'color':'#666'}).text('请输入新的提现密码');
	}).focusout(function(){
		var _this = $(this),
			v = _this.val();
		checkNewpwd(v);
	...
	$("#newpwd_confrim").focusin(function(){
		$("#js_newpwd_confrim_tips").css({'color':'#666'}).text('请再次确认新的提现密码');
	}).focusout(function(){
		var _this = $(this),
			v = _this.val();
	...
	$("#modifyPayPwdBtn").click(function(){
		...
		if(checkOldpwd(oldpwd) && checkNewpwd(newpwd) && checkNewpwdConfirm(newpwd2)){
			...
			obj.newpwd = newpwd;
			$.ajax({
				url: '__GROUP__/Paypwd/verifynew',
				type: 'post',
				data: obj,
				dataType: 'json',
				beforeSend: function(){
					$("#modifyPayPwdBtn").attr('disabled', true);
				},
				success: function(p){
					$("#modifyPayPwdBtn").attr('disabled', false);
