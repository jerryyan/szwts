...
<div id="platform_add_panel">
	<div id="platform_add_layout">
		<div title="平台基本信息(1)" style="padding:10px 20px;">
			...
			<div class="fitem">
				<label>平台名称：</label>
				<input name="name" type="text" maxlength="15" size="15" />
			</div>
			<div class="fitem">
				<label>安全评级：</label>
				<select name="grade">
		...
		<div title="平台基本信息(2)" style="padding:10px 20px;">
			<div class="fitem">
				<label>官方网址：</label>
				<input name="website" type="text" size="35" />
			</div>
			<div class="fitem">
				<label>注册资金：</label>
				<input name="injection" type="text" size="25" />
			</div>
			<div class="fitem">
				<label>所在地区：</label>
			...
			<div class="fitem">
				<label>上线时间：</label>
				<input name="online_time" type="text" class="date mr20 Wdate" onclick="WdatePicker();" style="color:#666;" />
			</div>
			<div class="fitem">
				<label>ICP备案：</label>
				<input name="icp" type="text" size="25" />
			</div>
			<div class="fitem">
				<label>联系电话：</label>
		...
		<div title="平台管理费用" style="padding:10px 20px;">
			<div class="fitem">
				<label>管理费：</label>
				<input name="management_fee" type="text" size="35" />
			</div>
			<div class="fitem">
				<label>vip费：</label>
				<input name="vip_fee" type="text" size="30" />
			</div>
			<div class="fitem">
				<label>充值费：</label>
				<input name="recharge_fee" type="text" size="38" />
			</div>
			<div class="fitem">
				<label>提现费：</label>
				<input name="cash_fee" type="text" size="45" />
			</div>
		</div>
		<div title="公司简介" style="padding:10px 20px;">
			<div style="padding:5px;">
				<textarea name="introduction" style="width:500px;height:300px;"></textarea>
			</div>
		</div>
		<div title="数据请求接口" style="padding:10px 20px;">
			<div class="fitem_2">
				<label>在投标列表：</label>
				<input name="interface_list" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>在投数,利率及期限：</label>
				<input name="interface_count" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>图表统计：</label>
				<input name="interface_charts" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>最低,最高及平均收益：</label>
				<input name="interface_avg" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>注册绑定账户：</label>
				<input name="interface_reg_bind" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>绑定现有账户：</label>
				<input name="interface_login_bind" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>客户投资记录：</label>
				<input name="interface_tender" type="text" size="55" />
			</div>
			<div class="fitem_2">
				<label>更新交易状态：</label>
				<input name="interface_update" type="text" size="55" />
			</div>
		</div>
	</div>
	<div style="margin:5px 30px;">
		<a id="addPlatformButton" class="easyui-linkbutton" iconCls="icon-save">保存</a> 
	</div>
</div>

<script>
$("#platform_add_layout").tabs();
$.loadEditor($("#platform_add_panel").find("[name='introduction']"));

$("#addPlatformButton").click(function(){
	var obj = {},
		panel = $("#platform_add_panel");
	...
	obj.online_time = $.trim(panel.find("input[name='online_time']").val());
	...
	$.post('__URL__/save', obj, function(data){
		if(data>0){
			alert("操作成功~");
			$("#platform_add_panel").parent().window('close');
