<?php
class User_sendemail_logModel extends CommonModel{

	//获取平台邮件通知日志记录
	public function getList($obj){
		$select = " p1.id,p1.title,p1.status,p1.module,p1.email,from_unixtime(p1.addtime) as atime,p1.addip,p2.username,p2.realname ";
		$sql = " select $select from `{$this->tablePrefix}user_sendemail_log` as p1 left join `{$this->tablePrefix}user` as p2 on p1.user_id=p2.user_id where 1=1 ";
